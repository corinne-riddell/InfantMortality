...
second <- ggplot(subset(both2, Gender == "Both Genders" & Year >= 2010), aes(x = Year, y = RR))  + 
  geom_line(lwd = 1.5) + geom_ribbon(aes(ymin = LCL_RR, ymax = UCL_RR), col = "grey", alpha = 0.5) + 
  ylab("Risk Ratio") +
...
third <- ggplot(subset(both2, Gender == "Both Genders" & Year >= 2010), aes(x = Year, y = RD))  + 
  geom_line(lwd = 1.5) + geom_ribbon(aes(ymin = LCL_RD, ymax = UCL_RD), col = "grey", alpha = 0.5) +
