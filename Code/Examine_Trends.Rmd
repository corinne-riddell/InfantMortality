---
title: Has the difference between infant mortality rates in blacks and whites continued to narrow? A look back at the past ten years.
...
Current word count (text only, exclusive of title, references, and tables): XXX
Date of last revision: Dec XX, 2016

**Introduction**

Infant mortality decreased by 16% for infants born to non-Hispanic black women between 2005 and 2011 and 12% for infants born to non-Hispanic white women over the same time period.^1^ During this time, mortality declined for each of the top four causes of death.^1^ Understanding whether infant mortality has continued to decline since 2011, and whether the difference between blacks and whites continued to narrow will provide an important gauge of the United States' progress towards reducing racial inequality.

*Alternatively, I could say something like: 2015 was the first year to see a decrease in life expectancy since 1993. However, we noticed that infant mortality -- which makes an important contribution to overall life expectancy -- had gone up for blacks while decreasing for whites between 2014 and 2015. In this research letter, we consider the last ten years of infant mortality data, to explore how much progress has been made to narrowing the difference in infant mortality between blacks and whites.*

**Methods**

We abstracted data on deaths and population among infants from the US National Vital Statistics System by cause of death and race for 2005-2015. We limited our analysis to non-hispanic black and white infants.^2^ The overall rate of infant mortality was calculated as the number of deaths divided by the population size, according to race, year, and cause of death. We calculate these rates for the top four causes of infant death and for a residual category encapsulating all other causes. The excess risk of infant mortality among blacks was calculated as the absolute difference between the rates in blacks and whites. We do not include confidence intervals, as these rates are not subject to sampling error. R version 3.2.4 was used to conduct this analysis. No ethical review was required since this study used de-identified data.

**Results**

From 2005 to 2012, the infant mortality rate for non-hispanic blacks decreased from 14.3 to 11.1 per 1,000 infants (**Figure 1a**). Thereafter, the infant mortality rate in blacks plateaued, with two of the past three years seeing slight increases to the rate of infant mortality. For non-hispanic whites, the infant mortality rate decreased from 5.9 to 4.9 per 1,000 infants from 2005 to 2015 and experienced no years of increasing rates. Because the black infant mortality rate declined faster than the white rate, the excess risk in blacks fell from 8.4 excess deaths per 1,000 infants in 2005 to 6.0 excess deaths per 1,000 in 2012 (**Figure 1b**). Thereafter, excess risk rose slightly to 6.3 in 2015. 

An excess risk of 6.3 deaths per 1,000 infants implies nearly 4,000 excess infant deaths among blacks compared to the amount of deaths that would have occurred had they experienced the mortality rate among whites. To arrive at this number, the black infant mortality rate that occurred was 6907/614972, the number of deaths and population size among black infants in 2015. To calculate excess deaths, we then apply the rate of mortality among white infants (4.9 per 1,000) to the black population, and see that 3041 black infants would have died rather than 6907 -- an excess of 3866 deaths. 

If the rate of death was held at its 2014 level (6698 deaths to 608099 infants), 133 fewer black infant deaths would have occurred.
...
total4 <- im.cod.dat2 %>% group_by(Race, Year) %>%
  filter(cod_recode %in% c("Congenital Malformations", "SIDS", "Short Gestation/Low Birthweight", "Maternal Complications")) %>%
...
third <- ggplot(subset(both2, Gender == "Both Genders" & Year >= 2005), aes(x = Year, y = RD))  + 
  ...
  ylab("Excess risk per 1,000 infants") + 
...
**Figure 2** shows trends in cause-specific mortality for the four leading causes of infant death and all other causes. Black infants have a much higher rate of deaths attributed to short gestation/low birthweight. Between 2005 and 2011, these deaths decreased for blacks, but have plateaued in recent years. The absolute differences between blacks and whites in the rates of the other leading causes are smaller than for short gestation/low birthweight. For these causes, rates among blacks and whites decreased between 2005 and 2015, although deaths related to both SIDS and congenital malformations increased for blacks last year. There is a large difference in deaths due to all remaining causes between blacks and whites. Between 2005 and 2011, there was sustained progress towards reducing mortality among blacks, but this progress has stalled in recent years, and last year also saw an increases in deaths from all causes for blacks.

**Comment**

There was sustained progress towards reducing the excess risk of infant mortality in blacks compared with whites between 2005 and 2011, followed by a period of little to no reduction in black infant mortality. In those years, there was no progress towards reducing the excess of deaths in blacks related to short gestation/low birthweight, congenital malformations, and all other causes (not including SIDS and deaths related to maternal complications of pregnancy and delivery).

To construct infant mortality rates, our analysis used data on the population of infants in each year as opposed to linked birth/infant death files which were unavailable for 2014 and 2015.

**Figure 1: Infant mortality and excess risk of death for blacks compared with whites, United States, 2005-2015**
```{r all_three, fig.width=7, fig.height=3.5, echo=F}
#grid.arrange(first, second, third, nrow = 3, heights = c(0.5, 0.25, 0.25))
grid.arrange(first, third, ncol = 2)
```

**Figure 2: Trends in COD-specific mortality for the top four causes of infant death, United States, 2005-2015**

```{r COD_trends_IMR, echo=F, fig.width=7, fig.height=7, warning=F}
a <- ggplot(subset(both.cod, Year >= 2005 & cod_recode!= "All other causes"), aes(x = Year, y = Black_IMR*100)) + 
  #geom_ribbon(aes(ymin= Black_IMR, ymax = White_IMR), col = "grey", alpha=0.2) +
  geom_segment(data = subset(both.cod, Year %in% c(2005, 2012, 2015) & cod_recode!= "All other causes"), aes(yend = Black_IMR*100, y = 0, xend = Year), lty = 3) +
  geom_line(aes(col = "Black")) + facet_wrap(~cod_recode2) +
  geom_line(aes(y = White_IMR*100, col = "White")) +   
  theme_bw() + ylab("Rate (per 100,000)") + theme(legend.position = "top") +
  scale_color_manual(values = c("#b2182b", "#2166ac"), guide = guide_legend(title = "")) +
  scale_y_continuous(limits = c(-10, 4*100), breaks = seq(from=0, to=4*100, by = 1*100), minor_breaks = seq(from=0, to=4*100, by = 0.5*100)) +
...
```{r COD_trends_IMR_other, echo=F, fig.width=3.75, fig.height=3.5, warning=F}
b <- ggplot(subset(both.cod, Year >= 2005 & cod_recode== "All other causes"), aes(x = Year, y = Black_IMR*100)) + 
  ...
  scale_y_continuous(limits = c(-10, 8*100), breaks = seq(from=0, to=8*100, by = 1*100), minor_breaks = seq(from=0, to=8*100, by = 0.5*100)) +
  scale_x_continuous(limits = c(2004, 2016), breaks = seq(from=2005, to=2015.5, by = 2)) +
  geom_text(data = subset(both.cod, Year %in% c(2005, 2012, 2015) & cod_recode== "All other causes"), 
            aes(x = Year, y = (Black_IMR + 0.3)*100, label = round(Black_IMR*100,1)), col = "#b2182b") +
  geom_text(data = subset(both.cod, Year %in% c(2005, 2012, 2015) & cod_recode== "All other causes"), 
...
Dr. Riddell had full access to all of the data and takes responsibility for the integrity of the data and the accuracy of the analysis. A replication data set including the raw data and statistical code to reproduce the paper results is publicly available on github: https://github.com/corinne-riddell/InfantMortality/.
...
1. [ref NCHS data brief # 120]. https://www.cdc.gov/nchs/data/databriefs/db120.pdf
...
```{r COD_RiskRatio, fig.width=8, fig.height=6, echo=F, eval = F}
...
```{r COD_RiskDifference, fig.width=9, fig.height=6, echo=F, eval = F}
...
```{r, fig.width=6.5, fig.height=4.5, echo=F, eval = F}
...
```{r, fig.width=6, fig.height=4.5, echo=F, eval = F}
